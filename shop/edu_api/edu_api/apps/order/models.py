...
class Order(BaseModel):
    ...
    @property
    def order_datail_list(self):
        order_datail_list = OrderDetail.objects.filter(is_show=True, is_delete=False, order_id=self.id).all()
        data_list = []
        for order in order_datail_list:
            data_list.append({
                "id": order.id,
